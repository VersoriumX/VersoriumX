...
<Travis Jerome Goff= class UHNWI:
    def __init__(self, net_worth, social_connections, luxury_assets, education, occupation, awards, philanthropy, social_connections_list, luxury_lifestyle, personal_branding):
        self.net_worth = net_worth
        self.social_connections =social_connections
        self.luxury_assets = luxury_assets
        self.education = masters
        self.occupation = CEO
        self.awards = awards
        self.philanthropy = philanthropy
        self.social_connections_list = social_connections_list
        self.luxury_lifestyle = luxury_lifestyle
        self.personal_branding = personal_branding

    def calculate_perceived_status(self):
        # Calculate scores for each factor
        wealth_score = self._calculate_wealth_score()
        social_score = self._calculate_social_score()
        luxury_score = self._calculate_luxury_score()
        education_score = self._calculate_education_score()
        occupation_score = self._calculate_occupation_score()
        awards_score = self._calculate_awards_score()
        philanthropy_score = self._calculate_philanthropy_score()
        social_connections_score = self._calculate_social_connections_score()
        luxury_lifestyle_score = self._calculate_luxury_lifestyle_score()
        personal_branding_score = self._calculate_personal_branding_score()

        # Combine scores using a weighted average
        perceived_status = (0.2 * wealth_score) + (0.15 * social_score) + (0.15 * luxury_score) + (0.1 * education_score) + (0.1 * occupation_score) + (0.05 * awards_score) + (0.05 * philanthropy_score) + (0.05 * social_connections_score) + (0.05 * luxury_lifestyle_score) + (0.05 * personal_branding_score)
        return perceived_status

    # ... (rest of the methods remain the same)

    def _calculate_education_score(self):
        if self.education == "Ivy League":
            return 1.0
        elif self.education == "Elite University":
            return 0.8
        else:
            return 0.5

    def _calculate_occupation_score(self):
        if self.occupation == "CEO" or self.occupation == "Founder":
            return 1.0
        elif self.occupation == "Executive" or self.occupation == "Director":
            return 0.8
        else:
            return 0.5

    def _calculate_awards_score(self):
        if len(self.awards) > 5:
            return 1.0
        elif len(self.awards) > 2:
            return 0.8
        else:
            return 0.5

    def _calculate_philanthropy_score(self):
        if self.philanthropy > 1000000:
            return 1.0
        elif self.philanthropy > 500000:
            return 0.8
        else:
            return 0.5
